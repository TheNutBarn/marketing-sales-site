...
| Cart model `{id, name, price, quantity}` | Sound data model, migrate to React Context |
| Mailto checkout | Works for current volume; preserve as Phase 1 |
...
- Remove `alert()` for cart additions — use inline toast notification
- Add "Continue Shopping" after add-to-cart instead of forcing cart open
- Add "Order via Email" as a clearly labeled fallback, not hidden in cart flow
...
|---|---|---|
| Frontend framework | Next.js 14 (App Router) | Static generation + ISR, excellent Vercel integration |
| Styling | Tailwind CSS | Utility-first, consistent spacing, dark mode ready |
...
| E-commerce Phase 1 | mailto order flow | Preserved from current site, zero infrastructure |
| E-commerce Phase 2 | Stripe (via Next.js API routes) | Scaffolded but not live until ready |
| Type safety | TypeScript | Prevents runtime errors, better DX |
...
WPGraphQL API endpoint
    ↓ (Next.js fetch at build time / ISR)
Next.js pages (static HTML generated)
    ↓ (served from edge)
User's browser
    ↓ (form submission)
Next.js API Route (/api/contact)
    ↓ (email dispatch)
Nodemailer → thenutbarnllc@gmail.com
```
...
| Variable | Local (`.env.local`) | Dev (Vercel Preview) | Prod (Vercel) |
|---|---|---|---|
...
**Key principle:** No email address, API URL, or secret key is hardcoded anywhere in source. All are environment-driven.
...
- Cart sidebar/drawer (slide-in)
- "Order via Email" checkout with mailto link
- Stripe placeholder section: "Coming Soon: Pay by Card"

**Schema:** `Product` JSON-LD for each product
...
*Goal: Working Next.js project on Vercel, WordPress CMS live locally*

- [ ] Initialize Next.js 14 + Tailwind + TypeScript project in repo root
...
- [ ] Create `.env.local` with `WORDPRESS_API_URL=http://nutbarn-local.local/graphql`
...
- [ ] Confirm `pnpm dev` starts Next.js successfully
...
**Week 2:**
- [ ] Build `app/layout.tsx` — root layout, nav, cart provider, schema
- [ ] Build `app/page.tsx` — homepage with hero, products, story teaser, schedule strip
- [ ] Build `components/Header.tsx` and `components/Footer.tsx`
- [ ] Build `components/CartDrawer.tsx` with React Context cart state
- [ ] Build `lib/cart-context.tsx`

**Week 3:**
- [ ] Build `app/shop/page.tsx` — full product catalog, cart integration
- [ ] Build `app/our-story/page.tsx` — full story, brand values
- [ ] Build `app/find-us/page.tsx` — static schedule + WordPress events
- [ ] Build `lib/wordpress.ts` — mock data fallback + WPGraphQL fetch

**Week 4:**
- [ ] Build `app/contact/page.tsx` — contact form
- [ ] Build `app/api/contact/route.ts` — email dispatch via Nodemailer
- [ ] Build `app/faq/page.tsx` — FAQ with FAQPage schema
- [ ] Build `app/blog/page.tsx` — blog index (WP-powered)
- [ ] Build `app/blog/[slug]/page.tsx` — individual post page

**Week 5:**
- [ ] Build `app/wholesale/page.tsx`
- [ ] Build `app/api/checkout/route.ts` — Stripe stub (returns 501)
- [ ] Add all JSON-LD schema to appropriate pages
- [ ] QA: mobile + desktop, all pages, cart flow, mailto checkout
- [ ] Add `next.config.js` security headers (migrated from `netlify.toml`)
- [ ] Add redirects in `next.config.js` for any legacy Netlify routes
...
| Task | Why |
|---|---|
| Add a new page type | Requires new Next.js route and component |
...
9. Set in `.env.local`: `WORDPRESS_API_URL=http://nutbarn-local.local/graphql`
