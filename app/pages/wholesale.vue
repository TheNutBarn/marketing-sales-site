  ...
  <div>
    ...
    <section class="px-4 py-14 text-center" style="background:var(--gradient-hero);color:var(--color-text-inverse);">
      <div class="mx-auto" style="max-width:var(--container-md);">
        <p class="eyebrow mb-2" style="color:var(--color-mustard);">B2B</p>
        <h1 style="font-size:clamp(2rem,5vw,3.5rem);color:var(--color-cream-light);">Wholesale</h1>
    ...
    <div class="mx-auto px-4 py-12" style="max-width:var(--container-xl);">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
        <!-- Why wholesale -->
        <div>
          <h2 class="mb-6" style="color:var(--color-denim);">Who It's For</h2>
          <ul class="space-y-4">
        ...
        <!-- Details -->
        <div class="rounded-lg p-6 card-vintage" style="background-color:var(--color-cream-light);border:2px solid var(--color-denim);">
          <h2 class="mb-4" style="color:var(--color-denim);">Wholesale Details</h2>
      ...
      <div class="max-w-lg mx-auto">
        <h2 class="mb-6 text-center" style="color:var(--color-denim);">Send an Inquiry</h2>

        <div v-if="successMessage" role="status" class="mb-6 rounded-lg p-4 border" style="background-color:var(--color-success-light);border-color:var(--color-success);color:var(--color-success);">{{ successMessage }}</div>
        <div v-if="errorMessage" role="alert" class="mb-6 rounded-lg p-4 border" style="background-color:var(--color-error-light);border-color:var(--color-error);color:var(--color-error);">{{ errorMessage }}</div>
        ...
        <form @submit.prevent="submit" novalidate class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input id="ws-name" v-model="form.name" type="text" required autocomplete="name" class="w-full px-3 py-2.5 text-sm rounded border" style="border-color:var(--color-border);" placeholder="Jane Smith" />
            </div>
            <div>
              ...
              <input id="ws-company" v-model="form.companyName" type="text" required class="w-full px-3 py-2.5 text-sm rounded border" style="border-color:var(--color-border);" placeholder="Acme Gift Shop" />
            </div>
          </div>

          <div>
            <label for="ws-email" class="block text-sm font-bold mb-1">Email <span style="color:var(--color-error);" aria-hidden="true">*</span></label>
            <input id="ws-email" v-model="form.email" type="email" required autocomplete="email" class="w-full px-3 py-2.5 text-sm rounded border" style="border-color:var(--color-border);" placeholder="jane@acmegifts.com" />
          </div>

          <div>
            <label for="ws-phone" class="block text-sm font-bold mb-1">Phone</label>
            <input id="ws-phone" v-model="form.phone" type="tel" autocomplete="tel" class="w-full px-3 py-2.5 text-sm rounded border" style="border-color:var(--color-border);" placeholder="(517) 555-1234" />
          </div>

          <div>
            <label for="ws-type" class="block text-sm font-bold mb-1">Type of Business</label>
            <select id="ws-type" v-model="form.businessType" class="w-full px-3 py-2.5 text-sm rounded border bg-white" style="border-color:var(--color-border);">
          ...
          <div>
            ...
            <select id="ws-volume" v-model="form.estimatedVolume" class="w-full px-3 py-2.5 text-sm rounded border bg-white" style="border-color:var(--color-border);">
          ...
          <div>
            ...
            <textarea id="ws-message" v-model="form.message" rows="4" class="w-full px-3 py-2.5 text-sm rounded border resize-none" style="border-color:var(--color-border);" placeholder="Tell us about your business and what you're looking for..." />
          </div>

          <button type="submit" :disabled="submitting" class="w-full font-bold py-3 rounded btn-vintage disabled:opacity-60" style="background-color:var(--color-orange);color:white;">
